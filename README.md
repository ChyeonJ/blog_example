...
토큰 유효기간
- JWT의 이론을 공부하면서 클라이언트가 토큰을 가지고 있고 이것으로 여러 권한을 받아올 수 있다면 클라이언트의 토큰을 탈취한다면 너무 쉽게 뚫리지 않을까? 라는 생각을 했지만 리프레시 토큰이라는 기능이 있었다.

### 리프레시 토큰
1. 보안의 문제를 해결하려면 토큰의 exp를 짧게 두면 되겠다?
- 정답은 아니다. 결국에는 클라이언트는 토큰을 재발급 받기 위한 번거로움이 있어 사용성이 떨어진다
...
1. 인증요청 : 클라이언트가 서버에 인증 요청
2. 엑세스 토큰 & 리프레시 토큰 응답 : 서버는 클라이언트에 전달한 정보를 바탕으로 인증 정보가 유효한지 확인, 엑세스 토큰과 리프레시 토큰을 만들어 클라이언트에게 전달
3. 리프레시 토큰 저장(DB) : 서버에서 생성한 토큰을 DB에 저장
 - 무결성이 장점인 토큰이 DB에 저장되면 무결성의 장점이 있을까?
 - 결론으로 따지면 보안의 이점을 가져가 위해서 반 무결성 상태로 유지한다. DB에 저장하는 이유는 혹시나 하는 탈취 형태의 해킹이 발생 시 DB에서 즉시 삭제가 가능하기 때문이다.
4. 요청 : 인증을 필요로하는 API 호출할 때 클라이언트에 저장된 엑세스 토큰과 함께 API를 요청
5. 토큰 유효성 검사 & 응답 : 서버는 전달받은 엑세스 토큰이 유효한지 검사한 뒤 클라이언트 요청 처리
6. (만료된 엑세스 토큰과 함께)요청 : 엑세스 토큰이 만료된 뒤에 클라이언트에서 원하는 정보를 얻기 위해서 서버에 API 요청 보냄
7. 토큰 만료 응답 : 서버는 엑세스 토큰이 유효한지 검사 / 만료된 토큰이면 Exception처리
8. (리프레시 토큰)과 함께 액세스 토큰 발급 요청 : 저장해둔 리프레시 토큰과 함께 엑세스 토큰을 발급 요청
...
### OAuth란?
- 제3의 서비스 계정 관리를 맡기는 방식 ex) 네이버로 로그인, 구글로 로그인

### OAuth 용어 정리
1. 리소스 오너 : 자신의 정보를 사용하도록 인증 서버에 허가하는 주체, 서비스를 이용하는 사용자가 리소스 오너
2. 리소스 서버 : 리소스 오너의 정보를 가지며, 리소스 오너의 정보를 보호하는 주체, 네이버, 구글, 페이스북이 리소스 서버
3. 인증 서버 : 클라이언트에게 리소르 오너의 정보에 접근할 수 있는 토큰 발급하는 역할을 하는 애플리케이션
4. 클라이언트 애플리케이션 : 인증 서버에게 인증을 받고 리소스 오너의 리소스를 사용하는 주체

### OAuth 리소스 오너 정보 취득하는 4가지
1. 권한 부여 코드 승인 타입 : 클라이언트가 리소스에 접근하는데 사용, 권한에 접근할 수 있는 코드와 리소스 오너에 대한 액세스 토큰을 발급 받음
2. 암시적 승인 타입 : 서버가 없는 자바스크립트 웹 애플리케이션 클라이언트에서 주로 사용하는 방법
...
### ![OAuth 로직]()
1. 권한 요청 : 스프링 부트 서버가 특정 사용자 데이터에 접근하기 위해 권한 서버에 요청 요청 URI양식은 클라이언트 ID, 리다이렉트URL, 응답 타입등을 파라미터 전송
- ex)
...
- response_type : 클라이언트가 제공받길 원하는 응답 타입, 인증 받을 때는 code값을 포함
- scope : 제공받고자 하는 리소스 오너의 정보 목록

2. 데이터 접근용 권한 부여 : 인증 서버에 요청을 처음 보내는 경우 최초 1회 진행하여 접근 동의 취득, 이후는 인증 서버에서 동의 내용을 저장하여 로그인만 진행
3. 인증코드 제공 : 사용자가 로그인에 성공하면 권한 요청 시에 파라미터로 보낸 redirect_uri로 리다이렉션 파라미터 인증 코드 함께 제공
- ex)
- GET http://localhost:8080/myapp?code-a1s2f3mcj2
4. 액세서 토큰 응답 : 인증 코드 받으면 액세스 토큰 교환/ 액세스 토큰은 로그인 세션에 대한 보안 자격증명하는 식별 코드 의미
- ex)
- POST spring-authorization-server.example.com/token
    {
        ...
        "code" : "a1b2v3d4e5f6g7h8"
- client_secret : OAuth 서비스에 등록할 때 제공받는 비밀키
- grant_type : 권한 유형 확인하는데 사용 권한 서버는 요청 값을 기반으로 유효한 정보인지 확인, 유효한 정보라면 액세스 토큰으로 응답

5. 액세스 토큰으로 API 응답 & 반환 : 정보가 필요할 때마다 API호출을 통해 정보를 가져오고 리소스 서버 토큰이 유효한지 검사한 뒤 응답

- OAuth 2.0의 경우 이론만 진행하도록 하겠습니다.

---------------------------------------------------------------------------

# 4. AWS 배포 

EC2는 AWS에서 제공하는 클라우드 컴퓨팅 서비스 가상의 PC, 서버 한대를 임대하는 서비스

나의 서비스가 인기가 많아져서 사용자의 요청이 많아진다면 서버 한대로는 처리가 어렵기 때문에 여러 EC2가 필요하게됨
하지만 늘 EC2를 많이 준비할 수는 없는 상황이기에 ***오토 스케일링 그룹***을 사용하여 사용자의 요청 횟수에 따라 EC2를 늘이거나 줄임
